    ...
    def str_extract(self, lvalue_formatter):
        ...
        for f in reversed(self.subs):
            ...
            if pos == 0:
                ...
                ret = f'deposit{bitop_width}({ret}, {pos}, {bitop_width - pos}, {ext})'  # noqa: E501
            pos += f.len
        return ret
...
_ASMA_GEN_BODY: List[str] = list()
_ASMA_BASE_TO_ARG: Dict[str, str] = dict()


def _asma_func_body(inst_name: str) -> str:
    return '{ printf("%s\\n"); return true;}' % inst_name


def _asma_scoped_name(inst_name: str) -> str:
    global arch_prefix
    return arch_prefix + "::" + inst_name


def _asma_descoped(inst_name: str) -> str:
    return inst_name[len(arch_prefix + "::"):]
...
class Pattern(General):
    ...
    def output_decl(self):
        ...
        global _ASMA_GEN_BODY
        global _ASMA_BASE_TO_ARG

        output('typedef ', self.base.base.struct_name(),
               ' arg_', self.name, ';\n')
        _ASMA_BASE_TO_ARG.update({_asma_scoped_name(self.name):
                                  self.base.base.struct_name()})
        if _asma_scoped_name(self.name) in _ASMA_GEN_BODY:
            output(translate_scope, 'bool ', translate_prefix, '_', self.name,
                   '(DisasContext *ctx, arg_', self.name, ' *a);\n')
        else:
            output(translate_scope, 'bool ', translate_prefix, '_', self.name,
                   '(DisasContext *ctx, arg_', self.name, ' *a)',
                   _asma_func_body(self.name), "\n")
            _ASMA_GEN_BODY.append(_asma_scoped_name(self.name))
...
def main():
    ...
    cur_dir = cur_path[:len(cur_path) - len("decodetree-asma.py")]
    gen_json = os.path.join(cur_dir, "gen.json")
    if not os.path.exists(gen_json):
        _ASMA_GEN_BODY = list()
        _ASMA_BASE_TO_ARG = dict()
    else:
        with open(gen_json, "r") as fp:
            json_info = json.load(fp)
            _ASMA_GEN_BODY = json_info["gen_info"]
            _ASMA_BASE_TO_ARG = json_info["base_to_arg"]
    decode_scope = 'static '
    ...
    with open(os.path.join(cur_dir, "gen.json"), "w") as fp:
        json.dump({"gen_info": _ASMA_GEN_BODY,
                   "base_to_arg": _ASMA_BASE_TO_ARG},
                  fp, indent=2)
